...
def get_forward_data(filename, use_chunks=True) -> xr.Dataset:
    """
    OPT-5: Open with chunks={} for lazy dask-backed reads.
    This avoids loading entire files into RAM at __init__ time.
    Each worker only materialises the slices it actually needs.
    """
    chunks = {} if use_chunks else None  # {} = dask auto-chunking
    if filename.endswith(".nc") or filename.endswith(".nc4"):
        return xr.open_dataset(filename, chunks=chunks)
    else:
        return xr.open_zarr(filename, chunks=chunks)
...
def random_land_patch(
    landmask_np, ny, nx, patch_size=(336, 336), min_land_frac=0.05,
    max_tries=2000, rng=None,
):
    """
    OPT-3: Accept pre-loaded numpy landmask instead of an xarray Dataset.
    Returns (y_slice, x_slice) instead of three xarray subsets.
    The caller applies the slices — avoids creating xarray objects here.
    """
    if rng is None:
        rng = np.random.default_rng()
    ...
    # ── Integral-image approach for O(1) land-fraction queries ──
    # Instead of computing submask.mean() in every trial (O(ph*pw) each),
    # precompute a summed-area table so each query is O(1).
    integral = landmask_np.cumsum(axis=0).cumsum(axis=1).astype(np.float32)
    total_pixels = ph * pw
 
    def _land_frac(y0, x0):
        y1, x1 = y0 + ph, x0 + pw
    ...
    for _ in range(max_tries):
        y0 = rng.integers(0, ny - ph + 1)
        x0 = rng.integers(0, nx - pw + 1)
        if _land_frac(y0, x0) >= min_land_frac:
            return slice(y0, y0 + ph), slice(x0, x0 + pw)
 
    warnings.warn(
        f"random_land_patch: could not meet land frac >= {min_land_frac} "
        f"after {max_tries} tries; using central patch."
    )
    y0_c = (ny - ph) // 2
    x0_c = (nx - pw) // 2
    return slice(y0_c, y0_c + ph), slice(x0_c, x0_c + pw)

    ...
    for i_fn, ds in enumerate(list_ds_train):
        ...
        if ind_level is not None:
            try:
                ...
                ds_subset = ds[varname].isel(bottom_top=ind_level)
        else:
...
def residual_zscore_var(conf, varname, ind_level=None, flag_float64=False, skipna=False):
    ...
    if ind_level is not None:
        try:
            ...
            ds_mean = ds_mean.isel(bottom_top=ind_level)
            ds_std = ds_std.isel(bottom_top=ind_level)      
    ...
    for i_fn, ds in enumerate(list_ds_train):
        ...
        if ind_level is not None:
            try:
                ...
                ds = ds.isel(bottom_top=ind_level)
